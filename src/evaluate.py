...
def ndcg(pos_index, pos_len):
    ...
    Through normalizing the scores, users and their recommendation list results in the whole test set can be evaluated.
...
def evaluate(scores, data, args):
    ks = np.array(args.ks)
    train = data['train']
    test = data['test']
    scores[train[:, 0], train[:, 1]] = -np.inf
    topk = torch.topk(scores, max(ks), dim=1)[1].cpu().numpy()
    topk_list_test = []
    
    for u,i in test:
        mask = np.isin(topk[u], np.array([i]))
        topk_list_test.append(mask)
    topk_list_test = np.vstack(topk_list_test) 
    
    hr_res_test = hit(topk_list_test, np.ones(len(topk_list_test), dtype=int)).mean(axis=0)
    ndcg_res_test = ndcg(topk_list_test, np.ones(len(topk_list_test), dtype=int)).mean(axis=0)

    return hr_res_test, ndcg_res_test